...
> DSH 的**指令**资产库：存一次，`/p`、 `@`、管理页三处通用。常用指令不用再翻聊天记录，也不用反复手打。

## 定位：指令，不是约束

提示词分两类，本体插件只承接其一：

- **约束**——角色、规则、格式、行为准则，对一个会话或项目**持续生效**。这归 **AGENTS.md**（`~/.dsh/AGENTS.md` 全局、项目级、skill）管：每会话加载、有作用域层级、不占历史、可版本管理。约束写进 AGENTS.md，而不是塞进提示词库。
- **指令**——一次性、可复用的提示词，靠人**主动取用**。这是本插件的全部职责：存着、搜得到、带变量、可回滚，`/p` 命令查、`@` 插入、`send` 直发，用完即走，不常驻、不占每轮上下文。

所以本插件**不做**"会话级 system prompt 注入"：注入固定一段持续占用 token、又不如 AGENTS.md 原生的提示词，两头不讨好。提示词是**指令**，指令该由人主动取用，而不是挂在那里每轮重复生效。
